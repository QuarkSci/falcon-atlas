...
/** Whole-booster offset: dips down and drifts to a landing spot, then holds. */
function boosterOffset(t: number, out: T.Vector3) {
  if (t < MT.separation) return out.set(0, 0, 0)
  const u = step(MT.separation, MT.landing, t)
  const dip = -22 * Math.sin(Math.PI * u)
  const lateral = 14 * u
  return out.set(LATERAL_DIR.x * lateral, dip, LATERAL_DIR.z * lateral)
}

/** Whole-upper-stage offset: keeps climbing after separation. */
function upperOffset(t: number, out: T.Vector3) {
  if (t < MT.separation) return out.set(0, 0, 0)
  const u = step(MT.separation, 1, t)
  return out.set(0, 26 * u, 0)
}
